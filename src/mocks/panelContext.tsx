...
export const MockPanelProvider: React.FC<{
  ...
  if (contextOverrides?.slices) {
    const slicesMap = contextOverrides.slices;
    context = {
      ...context,
      slices: slicesMap,
      getSlice: <T,>(name: string): DataSlice<T> | undefined => {
        return slicesMap.get(name) as DataSlice<T> | undefined;
      },
      hasSlice: (name: string, scope?: 'workspace' | 'repository'): boolean => {
        const slice = slicesMap.get(name);
        if (!slice) return false;
        if (!scope) return true;
        return slice.scope === scope;
      },
 ...
 */
export const createMockPanelProps = (options?: {
  ...
  // Override repository info if provided
  if (options?.repositoryPath !== undefined || options?.repositoryName !== undefined) {
    context.currentScope = {
      ...context.currentScope,
      repository: options.repositoryPath
        ? {
            name: options.repositoryName || 'project',
  ...
  // Override slices if provided
  if (options?.slices) {
    const slicesMap = context.slices as Map<string, DataSlice>;
    Object.entries(options.slices).forEach(([name, data]) => {
      slicesMap.set(name, createMockSlice(name, data));
    });

    // Update getSlice to use the new map
    context.getSlice = <T,>(name: string): DataSlice<T> | undefined => {
      return slicesMap.get(name) as DataSlice<T> | undefined;
    };
    context.hasSlice = (name: string): boolean => {
      return slicesMap.has(name);
    };
  }
